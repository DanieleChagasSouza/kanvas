...
class TestCourseView(APITestCase):
    ...
    def test_can_remove_student_from_course_using_superuser_token(self):
        pivot = baker.make("students_courses.StudentCourse")
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.superuser_token)
        url = (
            self.BASE_URL
            + str(pivot.course_id)
            + "/students/"
            + str(pivot.student_id)
            + "/"
        )
        ...
        result_status_code = response.status_code
        message = (
            f"\n<{url}/> status code da rota está diferente de {expected_status_code}."
        )
        self.assertEqual(expected_status_code, result_status_code, message)

        expected_count = 0
        result_count = StudentCourse.objects.filter(student=pivot.student_id).count()
        message = f"<{url}> estudante não foi removido do curso corretamente."
        self.assertEqual(expected_count, result_count, message)

    def test_can_not_remove_student_from_course_using_common_user_token(self):
        pivot = baker.make("students_courses.StudentCourse")
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.common_user_token)
        url = (
            self.BASE_URL
    ...
    def test_can_not_remove_associated_student_course_using_superuser_token(
        self,
    ):
        pivot = baker.make("students_courses.StudentCourse")
        new_student = baker.make("accounts.Account")
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.superuser_token)
        url = (
            self.BASE_URL
            + str(pivot.course_id)
        ...
        result_status_code = response.status_code
        message = (
            f"<{url}> status code da rota está diferente de {expected_status_code}."
        )
        self.assertEqual(expected_status_code, result_status_code, message)

        expected_body = {"detail": "this id is not associated with this course."}
        result_body = response.json()
        message = f"\n<{url}/> corpo de resposta está diferente de {expected_body}."
        self.assertEqual(expected_body, result_body, message)
